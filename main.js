...
document.addEventListener("DOMContentLoaded", () => {
  // Warm the invitation page while the welcome screen is visible so its opening
  // transition does not pause for a navigation-time download.
  fetch("invitation.html", { cache: "force-cache" }).catch(() => {});
...
/* ===========================================================================
   SCRIPT AMPERSANDS
   =========================================================================== */
function styleVisibleAmpersands() {
  const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      if (!node.nodeValue.includes("&") || node.parentElement?.closest("script, style")) {
  ...
  textNodes.forEach((node) => {
    ...
    node.nodeValue.split("&").forEach((part, index, parts) => {
      ...
      if (index < parts.length - 1) {
        ...
        ampersand.className = "script-ampersand";
        ampersand.textContent = "&";
        fragment.append(ampersand);
      }
    });
...
function initEntryTapToOpen() {
  ...
  if (!entry || !openButton || !invitationFrame || !openingCurtain) return;
  ...
  const openInvitation = () => {
    ...
    // The full invitation has been loading in the background since the welcome
    // screen appeared. Keep a closed curtain visible until that document is ready.
    openingCurtain.classList.add("is-active");
    invitationFrame.classList.add("is-open");
    // This runs inside the visitor's actual click, which Chrome permits for
    // audio. The embedded page stays silent and never creates a second track.
    window.startWeddingMusic?.();

    // The iframe begins loading with the first page. Two animation frames make
    // sure the closed curtain is painted before its panels move apart.
    requestAnimationFrame(() => {
      requestAnimationFrame(() => openingCurtain.classList.add("is-revealing"));
    });
  };
...
function initScrollAnimations() {
  ...
  // --- SCENE 4: COUPLE REVEAL (ONE-TIME UPWARD ENTRANCE) ---
  ...
  coupleTl
    .from(".couple-heading-wrap", { opacity: 0, y: 24, duration: 0.55, ease: "power2.out" }, 0)
    .fromTo(".temple-arch-outer", 
      { y: 70, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.75, ease: "power3.out" },
      0.12
    )
    .from(".couple-caption-badge", { opacity: 0, y: 18, duration: 0.45, ease: "back.out(1.5)" }, 0.48)
    .from(".couple-quote-text", { opacity: 0, y: 20, duration: 0.55, ease: "power2.out" }, 0.38);
...
function initAudioSystem() {
  ...
  const sendInvitationMusicState = () => {
    const invitationFrame = document.getElementById("invitation-frame");
    invitationFrame?.contentWindow?.postMessage(
      `invitationMusicState:${isPlaying ? "playing" : "paused"}`,
      "*"
    );
  };
  ...
  window.startWeddingMusic = () => {
    playTrack();
    window.setTimeout(sendInvitationMusicState, 250);
  };

  window.addEventListener("message", (event) => {
    if (event.data === "requestInvitationMusicState") {
      sendInvitationMusicState();
    }
    if (event.data === "toggleInvitationMusic") {
      if (isPlaying) {
        muteTrack();
      } else {
        playTrack();
      }
      window.setTimeout(sendInvitationMusicState, 100);
    }
  });
  ...
  const onFirstTap = () => {
    // The landing page is only a visual cover. Its preloaded audio must remain
    // silent so the invitation is the sole track that starts after opening.
