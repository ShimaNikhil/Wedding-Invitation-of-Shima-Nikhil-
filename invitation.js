...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Run Subsystems
  // The welcome page has already completed the entry moment; show this page at once.
  ...
  if (sessionStorage.getItem("playWeddingMusic") === "true") {
    sessionStorage.removeItem("playWeddingMusic");
...
function initAudioSystem() {
  ...
  window.addEventListener("message", (event) => {
    if (event.data === "invitationMusicState:playing") {
      isPlaying = true;
      updateAudioControl(true);
    }
    if (event.data === "invitationMusicState:paused") {
      isPlaying = false;
      updateAudioControl(false);
    }
  });
  ...
  toggleBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    // The welcome page owns the single audio element. In the embedded
    // invitation this button is a remote mute/unmute control for that track.
    if (window.parent !== window) {
      window.parent.postMessage("toggleInvitationMusic", "*");
      return;
    }
    if (isPlaying) {
      muteTrack();
  ...
  if (window.parent !== window) {
    window.parent.postMessage("requestInvitationMusicState", "*");
  }

  // Music is started by the welcome-page click. Do not start another copy when
  // a visitor later taps anywhere inside this embedded invitation.
}
